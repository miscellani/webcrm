...
public class OperatorAddServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		request.getRequestDispatcher("/jsp/sec/operator_add.jsp").forward(request, response);
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		boolean mark = false;
		try {
			mark = adminInfoDao.addOneAdmin(adminInfo);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		if (mark){		
			request.setAttribute("result", "操作员添加成功");
		}else{
			request.setAttribute("result", "操作员添加失败");
		}
